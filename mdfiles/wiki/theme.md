...
先行研究では、複数台の仮想マシン上でWebサーバを用意し、処理を分散させることで
応答性を確保した分散webシステムを開発していましたが、仮想マシンでは動作が重く、クラウド環境では余計なコストがかかります。

そこで本研究では、仮想マシンに比べて負荷の少ないコンテナ技術を用いて、低コストなWebサーバの割り当てを目的としています。さらにオートスケールという手法を用いて、アクセス数の変化を追従して必要最小限のコンテナを割り当てる機能の開発を行います。
