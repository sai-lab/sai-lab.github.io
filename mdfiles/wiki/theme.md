...
[キャッシュサーバーを用いたオートスケーリング機能](#キャッシュサーバーを用いたオートスケーリング機能)  
...
## キャッシュサーバーを用いたオートスケーリング機能

近年、Webサービスの普及や利用者の増加によるアクセス数の増加に伴い、Webサーバへの過負荷が問題となっています。
先行研究では、常時起動している共有キャッシュサーバを利用して仮想マシン上に仮想キャッシュサーバを構築し、サーバ数を管理することで応答性を確保した分散webシステムというものが開発されました。

しかし、仮想マシンでは動作が重く、クラウド環境では余計なコストがかかります。

そこで本研究では、仮想マシンに比べて処理の少ないコンテナ技術を用います。
そのうえで負荷の監視＋動的な起動、停止を行うことでサーバ数を最適化するオートスケール機能の開発を行います。
