...
分散Webシステムとは、ユーザーからのアクセスを複数台用意したWebサーバ(キャッシュサーバ)に分散させることで、オリジンサーバへの負担の軽減を目的とするシステムです。
また、キャッシュサーバとは、webサーバのデータを複製しそのサーバに変わり応答するサーバの事を指します。

本研究では、先行研究で開発された分散webシステム上で、クラウド、ローカルサーバを使いキャッシュサーバーを構築します。そのうえで、コンテナを用いて負荷に応じてキャッシュサーバを増減させ、快適にwebを利用できるようなオートスケーリング機能の開発、実験とその評価を行っています。
